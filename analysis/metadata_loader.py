...
def aggregate_iteration(iteration_path: Path, dependent_var_name: str):
    ...
    for exp_dir in exp_dir_list:
        ...
        dependent_var = config[dependent_var_name]

        metadata[dependent_var_name] = dependent_var
    ...
    if not df.empty:
        cols = [dependent_var_name] + [col for col in df.columns if col not in dependent_var_name]
        df = df[cols]
        df = df.sort_values(by=dependent_var_name)
...
def aggregate_all_iterations(base_dir: Path, dependent_var_name: str):
    iteration_col_name = "iteration"

    iteration_dir_list = sorted([p for p in base_dir.iterdir() if p.is_dir() and p.name != "__pycache__"])
    dfs = []

    for iteration_dir in iteration_dir_list:
        print(f"Processing: {iteration_dir}")

        aggregated_metadata_path = aggregate_iteration(iteration_dir, dependent_var_name=dependent_var_name)
        df = pd.read_csv(aggregated_metadata_path)
        df[iteration_col_name] = iteration_dir.name.strip(iteration_col_name)
        dfs.append(df)

    df_all = pd.concat(dfs, ignore_index=True)
    target_cols = ["iteration", dependent_var_name]
    cols = target_cols + [col for col in df_all.columns if col not in target_cols]
    df_all = df_all[cols]
    df_all = df_all.sort_values(by=[iteration_col_name, dependent_var_name])
