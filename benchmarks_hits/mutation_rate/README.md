# Benchmark: Substitution (Mutation) Rate vs. F1 Score

## Purpose
This benchmark evaluates the effect of varying the substitution rate
(referred to as *mutation rate* in the code) on the resulting F1 score.
The objective is to assess how sensitive the method is to changes in
the substitution rate.

## Benchmark Design
The benchmark is organized into multiple independent iterations.
Within each iteration, the substitution rate is varied between 0.00 and 0.09
while all other parameters remain fixed. A total of 5 iterations are 
performed, with identical parameter settings across iterations.
...
1. Generates a configuration file and an executable to run the
   simulation with the specified substitution rate.
