# Benchmark: Reference Sequence Base Length vs Memory
...
The objective is to assess the impact of reference base length on memory usage.
...
different random seeds. In total, one iteration is performed.
...
```
python -m benchmarks_hits.sequence_length_memory_benchmark_new.main
```

After completion, analysis and plotting can be performed with:

```
python -m benchmarks_hits.sequence_length_memory_benchmark_new.analyze
```
...
- `sequence_length_vs_memory.pdf`  
  Final plot showing the relationship between reference base length and memory.

## Notes
The benchmark was executed using commit `f1a2ff01554a38d7f0b447513f5fc75f862552bb` of the `raxtax_extension`
repository.